...
describe('Testing BooleanType', () => {
	...
	it(`Testing setters`, () => {
		...
		expect(() => (u.ok = 'yes')).toThrow('Expecting "ok" to be boolean, got string');
		expect(() => (u.ok = 5)).toThrow('Expecting "ok" to be boolean, got number');
		expect(() => (u.ok = [])).toThrow('Expecting "ok" to be boolean, got array');
		expect(() => (u.ok = {})).toThrow('Expecting "ok" to be boolean, got object');
		expect(() => (u.ok = function () {})).toThrow('Expecting "ok" to be boolean, got function');
	});
	...
	it(`Testing not null`, () => {
		...
		expect(() => Scenario2.create({ok: null})).toThrow('Property "ok" should be boolean and never null');
		expect(() => Scenario2.create({ok: undefined})).toThrow(
			'Property "ok" has null for its default value, but it doesn\'t accept null. Either set "ok" in create() to be boolean or set its default value'
		);
	});
	...
	it(`Testing default value not null`, () => {
		...
		expect(() => Scenario4.create({ok: null})).toThrow('Property "ok" should be boolean and never null');
	});
	...
	it(`Testing setting boolean as string`, () => {
		...
		expect(Scenario4.create({ok: 'TruE'}).ok).toBeTruthy();
		expect(Scenario4.create({ok: 'FalsE'}).ok).toBeFalsy();
		expect(() => Scenario4.create({ok: ''})).toThrow('Can not assign empty string to non-nullable BooleanType property "ok"');
		expect(() => Scenario4.create({ok: 'falsy'})).toThrow('Expecting "ok" to be boolean, got string');
		...
		expect(() => (s4.ok = 'nothing')).toThrow('Expecting "ok" to be boolean, got string');
		expect(() => (s4.ok = 5)).toThrow('Expecting "ok" to be boolean, got number');
		...
		expect(() => Scenario3.create({ok: 'falsy'})).toThrow('Expecting "ok" to be boolean, got string');
		...
		expect(() => (s3.ok = 'nothing')).toThrow('Expecting "ok" to be boolean, got string');
		expect(() => (s3.ok = 7)).toThrow('Expecting "ok" to be boolean, got number');
		expect(() => (s3.ok = () => {})).toThrow('Expecting "ok" to be boolean, got function');
	...
	it(`Testing custom validator`, () => {
		...
		expect(() => Scenario5.create({ok: false}).ok).toThrow('Ok must always be true!');
		expect(() => {
			const u = Scenario5.create();
			u.ok = false;
		}).toThrow('Ok must always be true!');
		expect(() => {
	...
	it(`Testing invalid default value type`, () => {
		expect(() => Scenario6.create()).toThrow('Property "ok" should have boolean for its default value, got string');
	});
