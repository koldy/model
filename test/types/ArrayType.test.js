...
describe('Testing ArrayType', () => {
	...
	it(`Testing wrong type assignment errors`, () => {
		...
		expect(() => (u.list = 5)).toThrow('Expecting "list" to be array, got number');
		expect(() => (u.list = 0.12)).toThrow('Expecting "list" to be array, got number');
		expect(() => (u.list = '5')).toThrow('Expecting "list" to be array, got string');
		expect(() => (u.list = {})).toThrow('Expecting "list" to be array, got object');
		expect(() => (u.list = function () {})).toThrow('Expecting "list" to be array, got function');
	});
	...
	it(`Testing assignment on not null`, () => {
		...
		expect(u.list).toStrictEqual([]);
		expect(() => (u.list = null)).toThrow('Property "list" should be array and never null');
		expect(() => (u.list = undefined)).toThrow(
			'Property "list" has null for its default value, but it doesn\'t accept null. Either set "list" to be array or set its default value'
		);
	});

	it(`Testing invalid definition`, () => {
		expect(() => Scenario3.create()).toThrow('Default value in ArrayType must be type of array, got number');
	});
	...
	it(`Testing default value with not null`, () => {
		...
		expect(() => Scenario5.create({list: null}).list).toThrow('Property "list" should be array and never null');
		expect(Scenario5.create({list: undefined}).list).toStrictEqual([5]);
	});

	it(`Testing custom validator`, () => {
		expect(() => Scenario6.create()).toThrow('There should be at least 3 elements in array');
		expect(() => Scenario6.create({list: [1, 2, 3, 4, 5, 6]})).toThrow("There shouldn't be more than 5 elements in array");
